/**
 * Inline SVG bodies for the icons the lesson reader stamps into rendered
 * Markdown. The reader decorates HTML it did not author, so it cannot mount
 * Svelte icon components there; these are the same Lucide paths as the
 * components elsewhere (ISC licence), captured at build time from the package.
 */
...
/** One Lucide icon as a complete inline SVG, sized by the surrounding font. */
export function iconSvg(name: string, size = 16, extraClass = ''): string {
  const body = ICON_PATHS[name];
  if (!body) return '';
  return `<svg class="lesson-icon ${extraClass}" xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${body}</svg>`;
}
