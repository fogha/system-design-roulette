    ...
    <div class="workspace">
      ...
      <div class="editor">
        ...
        {#if route === 'cli'}
          ...
          {#if selected === 'custom' && draft}<label class="command">Custom command<input aria-label="Saved custom runner command" bind:value={draft.custom_command} placeholder={"'/path/to/agent' --print {prompt}"} /></label><p>Use {'{prompt}'} or append the prompt as the final argument.</p>{/if}
        {:else}
...
<style>
  ...
  .routes button { display: flex; align-items: center; gap: 7px; flex: 1; padding: 9px 7px; color: var(--muted); background: var(--bg); border: 1px solid var(--node-border); border-radius: 4px; font-size: 11px; cursor: pointer; } .routes .active { color: var(--accent); border-color: var(--accent); } .routes small { margin-left: auto; font: 9px var(--font-mono); opacity: .6; }
  .workspace { display: grid; grid-template-columns: 152px minmax(0,1fr); gap: 17px; }
  .providers { display: grid; align-content: start; gap: 4px; } .providers button { display: flex; align-items: center; gap: 8px; border: 1px solid transparent; padding: 9px 7px; border-radius: 4px; background: transparent; color: var(--muted); text-align: left; font-size: 11px; cursor: pointer; } .providers .editing { color: var(--text); background: var(--surface-2); border-color: var(--node-border); }
  ...
  .free-toggle { justify-self: start; display: flex; align-items: center; gap: 7px; background: transparent; border: 0; color: var(--text); font-size: 11px; padding: 0; cursor: pointer; } .free-toggle span { display: grid; place-items: center; width: 16px; height: 16px; border: 1px solid var(--node-border); color: var(--accent); }
