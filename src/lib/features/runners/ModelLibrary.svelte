...
<div class="model-library">
  <div class="line"><span class="eyebrow">MODEL SHORTLIST <b>{models.length}</b></span><button type="button" class="text-action" onclick={() => manualOpen = !manualOpen}>Add by ID</button></div>
  ...
  <div class="search-row"><div class="search"><Search size={13} /><input aria-label="Search model catalogue" placeholder="Search models by name or ID…" value={query} oninput={e => { query = e.currentTarget.value; page = 0; }} /></div><button type="button" class="small-action" disabled={loading} onclick={() => load(true)}>{loading ? 'Loading…' : 'Refresh'}</button></div>
  <div class="catalogue" aria-label="Model catalogue">
    ...
    {/each}
    {#if !results.count}<p class="empty">{loading ? 'Loading this provider’s catalogue…' : query ? 'No models match your search.' : 'No catalogue is available. Add a model by ID, or configure a provider key first.'}</p>{/if}
  </div>
  <div class="pagination"><span>{results.count} models{results.count ? ` · ${results.current * 5 + 1}–${Math.min((results.current + 1) * 5, results.count)}` : ''}</span><div><button type="button" class="small-action" aria-label="Previous model page" disabled={results.current === 0} onclick={() => page = results.current - 1}>←</button><span>{results.current + 1} / {results.pages}</span><button type="button" class="small-action" aria-label="Next model page" disabled={results.current + 1 >= results.pages} onclick={() => page = results.current + 1}>→</button></div></div>
...
<style>
  ...
  .chip { display: flex; align-items: center; gap: 7px; max-width: 100%; padding: 4px 7px; border: 1px solid var(--node-border); border-radius: 4px; color: var(--text); background: var(--bg); font: 10px/1.5 var(--font-mono); cursor: pointer; } .chip span { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
  .search-row,.manual { display: flex; gap: 7px; } .search { display: flex; align-items: center; gap: 7px; flex: 1; min-width: 0; border: 1px solid var(--node-border); padding-left: 9px; background: var(--bg); color: var(--muted); }
