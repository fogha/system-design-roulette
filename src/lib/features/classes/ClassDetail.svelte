      ...
      {#if visited.includes(item.id)}
        {#if item.id === 'overview'}
          <div class="overview">
            ...
            <div class="overview-grid"><section class="info-card"><span class="eyebrow mono">YOUR STARTING POINT</span><h4>{program.accepted_path ? `Lessons start at ${program.accepted_path.entry_label}` : 'Begin where you are'}</h4><p>{program.accepted_path ? `${program.accepted_path.earlier_topics} earlier ${program.accepted_path.earlier_topics === 1 ? 'topic is' : 'topics are'} set aside and ${program.accepted_path.refreshers} prerequisite ${program.accepted_path.refreshers === 1 ? 'refresher comes' : 'refreshers come'} first. Revision ${program.accepted_path.revision}, from ${program.accepted_path.route === 'diagnostic' ? 'a short check' : program.accepted_path.route === 'manual' ? 'the stage you chose' : 'starting at the foundations'}.` : 'Start with the foundations, choose a stage or take a short check to find your level.'}</p><button class="ghost mono-ghost" onclick={() => select('entry')}>{program.accepted_path ? 'View personal path' : 'Set starting point'}<ArrowRight size={13} /></button></section>{#if program.route}<section class="info-card route-card"><span class="eyebrow mono">YOUR ROUTE · REVISION {program.route.revision}</span><dl class="route-facts"><div><dt>Completed on your route</dt><dd>{program.route.required_done} / {program.route.required_total} required</dd></div><div><dt>Course coverage</dt><dd>{program.route.coverage_done} / {program.route.coverage_total} core</dd></div><div><dt>Demonstrated prior knowledge</dt><dd>{program.route.demonstrated} {program.route.demonstrated === 1 ? 'sample' : 'samples'}</dd></div><div><dt>Needs review</dt><dd>{program.route.needs_review} {program.route.needs_review === 1 ? 'topic' : 'topics'}</dd></div></dl>{#if program.route.stale}<p class="stale" role="alert">The curriculum changed since you accepted this path. Review your starting point before the next lesson; your history and saved work are kept.</p><button class="cta mono-cta" onclick={() => select('entry')}>Review starting point<ArrowRight size={13} /></button>{:else if program.route.next}<p><strong>Next:</strong> {program.route.next.title} · {program.route.next.reason}</p>{:else}<p>Nothing is left on your accepted route; the course's final outcome check remains.</p>{/if}{#if !program.route.stale}<button class="ghost mono-ghost" onclick={() => select('curriculum')}>Review the route<ArrowRight size={13} /></button>{/if}</section>{/if}<section class="info-card"><span class="eyebrow mono">WORKING ENVIRONMENT</span><p class="environment">{course.environment}</p>{#if course.prerequisite_courses.length}<p>Suggested preparation: {course.prerequisite_courses.map(id => courseDefinition(id)?.label ?? id).join(', ')} or equivalent experience.</p>{/if}<button class="ghost mono-ghost" onclick={() => select('curriculum')}>Explore curriculum<ArrowRight size={13} /></button></section></div>
