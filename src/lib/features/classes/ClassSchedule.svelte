<script lang="ts">
  ...
  import { Plus, Pencil, Trash2, Clock3, CalendarClock, ArrowRight } from 'lucide-svelte';
...
<div class="schedule">
  ...
  <div id={`${uid}-times-panel`} role="tabpanel" aria-labelledby={`${uid}-times`} hidden={view !== 'times'} tabindex="0">
    ...
    {#if editing}
      <div class="editor">
        ...
        <div class="actions"><button class="ghost mono-ghost" onclick={() => editing = false} disabled={busy}>Cancel</button><button class="cta mono-cta" onclick={saveSlot} disabled={busy || !slotDays.length}>{busy ? 'Saving…' : 'Save study time'}</button></div>
      </div>
    {/if}
  ...
  <div id={`${uid}-plan-panel`} role="tabpanel" aria-labelledby={`${uid}-plan`} hidden={view !== 'plan'} tabindex="0">
    ...
    <form onsubmit={event => { event.preventDefault(); void plan(false); }}>
      ...
      {#if preview}<div class="preview" class:stale={!currentPreview}><span class="mono">{currentPreview ? 'TIMETABLE PREVIEW' : 'PREVIEW OUT OF DATE'}</span><h4>{preview.total_weekly_minutes} minutes per week</h4><p>{!currentPreview ? 'Your preferences changed. Preview again before saving.' : preview.meets_target ? 'This timetable meets your weekly target.' : `This timetable is short of your ${preview.target_weekly_minutes} minute target.`}</p><ul>{#each preview.slots as slot}<li><strong class="mono">{time(slot.hour,slot.minute)}</strong><span>{days(slot.weekdays)}</span></li>{/each}</ul></div>{/if}
      <div class="actions"><button type="submit" class="ghost mono-ghost" disabled={busy || !validWindows}>{busy ? 'Working…' : 'Preview timetable'}</button><button type="button" class="cta mono-cta" disabled={busy || !currentPreview || !validWindows} onclick={() => plan(true)}>Save weekly schedule</button></div>
    </form>
  </div>
</div>
<style>
  ...
  .preview { border: 1px solid var(--led-ok); border-radius: var(--radius-panel); padding: 20px; margin-top: 20px; background: var(--bg); } .preview.stale { border-color: var(--accent); } .preview > span { font-size: 10px; color: var(--led-ok); } .preview.stale > span { color: var(--accent); } .preview h4 { margin: 12px 0 8px; } .preview ul { padding: 0; list-style: none; display: grid; gap: 8px; margin-bottom: 0; } .preview li { display: flex; gap: 12px; color: var(--muted); font-size: 12px; } .preview strong { color: var(--fg); font-weight: 400; }
