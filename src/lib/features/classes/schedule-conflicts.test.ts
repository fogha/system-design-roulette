...
function program(subject_id: string, label: string, enabled: boolean, session_minutes = 30): ClassroomProgramView {
  return { subject_id: subject_id as ClassroomProgramView['subject_id'], kind: 'engineering', label, native_label: '', short_code: 'X', enabled, agent: 'claude', model: 'sonnet', custom_agent_bin: '', prompt_profile: '', prompt_version: 'v1', session_minutes, learning_goal: '', target_weekly_minutes: 0, progress: 0, progress_label: '', completed: false, focus_policy: 'advisory' as const, language_progress: null };
}
