...
Publish all task-owned commits to `fogha:feat/frontend` and update PR #1 with the final scope and limitations. Preserve the user's staged pnpm files. The remaining work is still the full P4–P9 scope listed above, including native rendering/handoff verification when desktop capture is available, complete connected-model lesson checks, runtime/import and subject-adapter cutover, occurrences/overlap/focus ownership, shared lesson navigation and drafts, curriculum/evidence completion, and release gates. OpenAI/Anthropic live keys and final logo selection remain user-side decisions. No merge or release is authorized by this checkpoint.
