...
P2's shared class/lesson/session/entry-profile/path contracts and numbered migration runner remain outstanding. P3–P9 remain planned: placement/path revisions, unified navigation/runtime, broader migration fixtures, complete investigation/capstone labs, richer language variation and the full display/installer rebrand still need implementation. The temporary guard against changing a legacy language starting level protects the existing denominator; P5 replaces that limitation with explicit path revisions that preserve history. Durable language/classroom activity drafts beyond the shared exercise workspace remain part of the shared-runtime work.
