...
A single lesson can also leave the desk as a CSV file (`lesson_export.rs`, the `export_lesson_csv` command), written under `Documents/Principia Desk/lessons/` beside the profile exports. One flat table with a `kind` column holds the lesson's metadata, its `##` sections, resources, exercise (with the learner's draft and reflection), and one row per check question with the four choices, the learner's answer and its result; language lessons add phrase, dialogue and prompt rows. Correct answers and explanations are included only once the check has been submitted, so the file cannot serve as a key for a lesson still in progress. Fields are quoted per RFC 4180, the file opens with a byte-order mark, and any field a spreadsheet would run as a formula is prefixed with an apostrophe, since lesson text is model-written.
